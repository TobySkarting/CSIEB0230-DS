...
class Stack
{
	...
	template <class KeyType>
	friend  std::ostream& operator<<(std::ostream&, Stack<KeyType>&);
public:
	...
	void StackEmpty() { std::cout << "empty" << std::endl; };
	void StackFull() { std::cout << "full" << std::endl; };
	void Output();
private:
	int top;
	KeyType *stack;
